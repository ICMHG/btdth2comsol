...
class StackedDieSection(Section):
    ...
    def get_runtime_sections(self) -> List['Section']:
        """获取运行时区域"""
        # 这里需要实现具体的逻辑
        return [self]  # StackedDieSection继承自Section，所以self本身就是Section类型
    ...
    def from_json(self, json_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 加载堆叠芯片特有属性，支持BTD格式的字段名
            if "power_type" in json_data or "powerType" in json_data:
                power_type_val = json_data.get(
                    "power_type", json_data.get("powerType", 1))
                if power_type_val == 1:
                    self.power_type = PowerType.TOTAL_POWER
                elif power_type_val == 2:
                    self.power_type = PowerType.POWER_MAP

            self.powermap_file = json_data.get(
                "power_map_file", json_data.get("powermapFile", ""))
            self.use_gds = json_data.get(
                "use_gds", json_data.get("useGDS", False))
            self.gds_file = json_data.get(
                "gds_file", json_data.get("gdsFile", ""))
            self.stack_tier = json_data.get(
                "stack_tier", json_data.get("stackTier", 1))
            self.temp_file = json_data.get(
                "temp_file", json_data.get("tempFile", ""))
            self.is_flip = json_data.get(
                "is_flip", json_data.get("isFlip", False))
            self.material_string = json_data.get(
                "material_string", json_data.get("materialString", ""))
            self.tim_material = json_data.get(
                "tim_material", json_data.get("timMaterial", ""))
            self.tags = json_data.get("tags", "")
            self.total_power = json_data.get(
                "total_power", json_data.get("totalPower", 0.0))
            self.max_die_temp = json_data.get(
                "max_die_temp", json_data.get("maxDieTemp", 0.0))
            self.scale_factor = json_data.get(
                "scale_factor", json_data.get("scaleFactor", 1.0))
            self.tim_size_x = json_data.get(
                "tim_size_x", json_data.get("timSizeX", 0.0))
            self.tim_size_y = json_data.get(
                "tim_size_y", json_data.get("timSizeY", 0.0))
            self.face_up = json_data.get(
                "face_up", json_data.get("faceUp", True))
            self.power_scale = json_data.get(
                "power_scale", json_data.get("powerScale", 1.0))
